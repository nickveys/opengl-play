...
private:
    ...
    /**
      ...
      */
    static inline bool _hasDynamicSize()
    { return false; }
...
public:
    ...
    Matrix( int size )
    {
        EIGEN_ASSERT_NOABORT( size == Size );
    }
...
protected:
    ...
    /** This array stores the size*size entries of the matrix.
      ...
      * In other words, the matrix entries are stored in the following
      * order:
      *
      * ( 0             size      ...     size*(size-1)    )
      * ( 1             heigth+1    ...     size*(size-1)+1  )
      * ( 2             size+2    ...     size*(size-1)+2  )
      *   ...           ...         ...     ...
      * ( size-1      size*2-1  ...     size*size-1      )
      */
    T m_array[ Size * Size ];
...
public:
    ...
    /** \hide */
    inline MatrixX & operator = ( const MatrixX & other )
    { return Base::operator = ( other ); }
    ...
    inline MatrixX& operator *=( const T & factor )
    { return Base::operator *= ( factor ); }
