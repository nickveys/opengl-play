...
{
    friend class LinearSolverBase< T, MatrixType,
                                   VectorType, LUDecompositionBase< T, MatrixType,
                                   VectorType, IntVecType> >;
...
public:
    ...
    /**
      ...
      */
    inline MatrixType & LU() { return m_LU; }
    inline const MatrixType & LU() const { return m_LU; }
    ...
    /**
      ...
      */
    inline IntVecType & P() { return m_P; }
    inline const IntVecType & P() const { return m_P; }
    ...
    /**
      ...
      */
    inline IntVecType & Q() { return m_Q; }
    inline const IntVecType & Q() const { return m_Q; }
...
( const MatrixType & A )
{
    ...
    if( dim() == 1 )
    {
        ...
        m_dimKer = ( Util::abs(m_biggestEigenValueU) == static_cast<T>(0) );
    ...
    for( int k = 0; k <= dim() - 2; k++)
    {
        ...
        T lu_k_k = LU()(k, k);
        if( ! Util::isNegligible( Util::abs(lu_k_k), Util::abs(biggestEntry) ) )
        {
    ...
    for( int k = 0; k <= dim() - 1; k++)
        if( Util::abs( LU()( k, k ) ) > Util::abs( m_biggestEigenValueU ) )
    ...
    m_dimKer = 0;
    for( int k = 0; k <= dim() - 1; k++)
        m_dimKer += Util::isNegligible(
                        Util::abs( LU()( k, k ) ),
                        Util::abs( m_biggestEigenValueU )
                                      );
}
...
( const VectorType & v, VectorType * result ) const
{
    ...
    for( int i = dim() - 1; i >= 0; i--) // i must be signed!
    {
        ...
        if( Util::isNegligible( Util::abs( *denomptr ),
                                Util::abs(m_biggestEigenValueU) ) )
        {
            // if the current vector coord is also zero
            if( Util::isNegligible( Util::abs( a[i] ),
                Util::abs(m_biggestEigenValueU) ) )
...
{
    EIGEN_ASSERT_NOABORT( result->size() == dim() );
    if( ! isInvertible() ) return false;
